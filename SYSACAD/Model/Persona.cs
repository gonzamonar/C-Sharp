...
namespace Model
{
    public abstract class Persona : Usuario
    {
        ...
        public Persona(string id, string nombre, string apellido, string dni, string genero, string username, string password, EPermiso credencial)
        : base(id, username, password, credencial)
        {
            this.nombre = nombre;
            this.apellido = apellido;
            int.TryParse(dni, out this.dni);
            this.genero = CastEGenero(genero);
        }
